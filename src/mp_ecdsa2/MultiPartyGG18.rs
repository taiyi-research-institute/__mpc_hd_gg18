...
impl Keys {
    ...
    pub fn phase2_verify_vss_construct_keypair_phase3_pok_dlog(
        ...
        Ok(KeygenDlog {
            shared_keypair: SharedKeyPair { y, x_i },
            dlog_proof,
        })
    }
    ...
    pub fn update_commitments_to_xi(
        ...
        let li: Scalar<Secp256k1> =
            VerifiableSS::map_share_to_new_params(self.member_id, &key_providers).catch_()?;
        Ok(comm * &li)
    }
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct KeygenDlog {
    pub shared_keypair: SharedKeyPair,
